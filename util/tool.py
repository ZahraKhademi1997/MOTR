...
#     for k in state_dict:
#         k = k.replace('pos_cross_attention', 'transformer.pos_cross_attention')
...
def load_model(model, model_path, optimizer=None, resume=False,
    ...
    # Adapt keys in state_dict and handle discrepancies
    adapted_state_dict = {}
    for k in state_dict:
        new_key = k.replace('pos_cross_attention', 'transformer.pos_cross_attention')  # Specific key adaptation
        if new_key in model_state_dict:
            if state_dict[k].shape == model_state_dict[new_key].shape:
                adapted_state_dict[new_key] = state_dict[k]
            else:
                print(f'Shape mismatch for {new_key}: expected {model_state_dict[new_key].shape}, got {state_dict[k].shape}')
        else:
            print(f'Key {new_key} not found in the model.')

    # Load adapted state dict
    model.load_state_dict(adapted_state_dict, strict=False)

    # Resume optimizer parameters if necessary
    if optimizer is not None and resume:
        if 'optimizer' in checkpoint:
            optimizer.load_state_dict(checkpoint['optimizer'])
            start_epoch = checkpoint['epoch']
