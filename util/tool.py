...
def load_model(model, model_path, optimizer=None, resume=False, lr=None, lr_step=None):
    ...
    for k in list(state_dict.keys()):
        if k in model_state_dict:
            if state_dict[k].shape != model_state_dict[k].shape:
                print(f'Skipping parameter {k} due to shape mismatch: checkpoint {state_dict[k].shape} vs model {model_state_dict[k].shape}')
                del state_dict[k]
        else:
            print(f'Dropping parameter {k} as it is not in model state_dict')
            del state_dict[k]

    for k in model_state_dict:
        if k not in state_dict:
            print(f'Adding missing parameter {k} from model state_dict')
            state_dict[k] = model_state_dict[k]
