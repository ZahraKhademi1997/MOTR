...
class Detector(object):
    ...
    def write_results(txt_path, mask_statistics, frame_id, bbox_xyxy, masks, identities):
        ...
        with open(txt_path, 'a') as f:
            for xyxy, mask, track_id in zip(bbox_xyxy, masks, identities):
                ...
                binary_mask = mask > 0.05 #test
                # binary_mask = mask > 0.3 #train
                # mask = binary_mask
                ...
                mask_filtered = gaussian_filter(connected_component_mask , sigma=0.1)
    ...
    def detect(self, prob_threshold=0.4, area_threshold=100, vis=False):
        ...
        for i in tqdm(range(0, self.img_len)):
            ...
            dt_instances = deepcopy(track_instances.to(torch.device('cpu')))
