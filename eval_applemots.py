...
def generate_unique_colors(identities):
    """
    Generates a unique color for each identity.
    Assumes identities is a 1D tensor of integers.
    """
    # Ensure identities are converted to integers if they are tensors
    identities_int = identities.numpy().astype(int) if isinstance(identities, torch.Tensor) else identities
...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'APPLE_MOTS/train/images', self.seq_num, 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'APPLE_MOTS/train/images', self.seq_num, 'img1', _) for _ in img_list if
    ...
    def load_img_from_file(self, f_path):
        ...
        def visualize_annotations(image, targets):
            ...
            for box, mask in zip(targets['boxes'], targets['masks']):
                ...
                # Create a color mask with an alpha channel of 0.5 (semi-transparent)
        ...
        # mask_path = f_path.replace('images', 'masks_with_ids').replace('.png', '.txt').replace('.jpg', '.txt')
    ...
    def visualize_img_with_bbox(img_path, img, dt_instances: Instances, identities=None, ref_pts=None, gt_boxes=None):
        ...
        # if dt_instances.masks is not None:
        #         img_show = visualize_img_with_masks(img, dt_instances.masks, dt_instances.obj_idxes, alpha=0.5)
