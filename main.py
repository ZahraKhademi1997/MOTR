...
def get_args_parser():
    ...
    parser.add_argument('--lr_mlp_names', default=["mlp"], type=str, nargs='+')
    parser.add_argument('--lr_mlp', default=2e-4, type=float)
    
    parser.add_argument('--lr_dynamic_encoder_names', default=["dynamic_encoder"], type=str, nargs='+')
    parser.add_argument('--lr_dynamic_encoder', default=2e-4, type=float)

    parser.add_argument('--lr_mask_positional_encoding_names', default=["mask_positional_encoding"], type=str, nargs='+')
    parser.add_argument('--lr_mask_positional_encoding', default=2e-4, type=float)
...
def main(args):
    ############################################################################
    # () Logging
    writer = SummaryWriter(log_dir='/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask_AppleMOTS-InstanceAware/outputs/logs/logs_loss')
    ############################################################################
    ...
    model.to(device)
    # output_dir = "/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-AppleMots-MaskFormer-Instance/outputs/model.txt"
    ...
    param_dicts = [
        {
            "params":
                ...
                #  and not match_name_keywords(n, args.lr_segmentation_mask_head_embedding_names)
                ...
                #  and not match_name_keywords(n, args.lr_mlp_names)
                and not match_name_keywords(n, args.lr_dynamic_encoder_names)
                and not match_name_keywords(n, args.lr_mask_positional_encoding_names)
        ...
        {
            "params": [p for n, p in model_without_ddp.named_parameters() 
                       if match_name_keywords(n, args.lr_dynamic_encoder_names) and p.requires_grad],
            "lr": args.lr * args.lr_dynamic_encoder,
            # "weight_decay": args.linear_proj_mult_weight_decay,
        },
        {
            "params": [p for n, p in model_without_ddp.named_parameters() 
                       if match_name_keywords(n, args.lr_mask_positional_encoding_names) and p.requires_grad],
            "lr": args.lr * args.lr_mask_positional_encoding,
            # "weight_decay": args.linear_proj_mult_weight_decay,
        },
        {
            "params": [p for n, p in model_without_ddp.named_parameters() 
                       if match_name_keywords(n, args.lr_seg_branches_names) and p.requires_grad],
            "lr": args.lr * args.lr_seg_branches,
            # "weight_decay": args.linear_proj_mult_weight_decay,
        },
        # {
        #     "params": [
        ...
        #     "params": [
        #         p for n, p in model_without_ddp.named_parameters()
        #         if match_name_keywords(n, args.lr_segmentation_mask_head_embedding_names) and p.requires_grad
        #     ],
        #     "lr": args.lr_segmentation_mask_head_embedding,
        #     # "weight_decay": args.mask_head_weight_decay, 
        # },
        # {
        #     "params": [
...

        {
            "params": [
                p for n, p in model_without_ddp.named_parameters()
                if match_name_keywords(n, args.lr_mlp_names) and p.requires_grad
            ],
            "lr": args.lr_mlp,
            # "weight_decay": args.mask_head_weight_decay, 
        },
    ...
    #         "params": [
    #             p for n, p in model_without_ddp.named_parameters()
    #             if match_name_keywords(n, args.lr_segmentation_bbox_attention_names) and p.requires_grad
    #         ],
    #         "lr": args.lr_segmentation_bbox_attention, 
    #         # "weight_decay": args.bbox_attention_weight_decay,
    #     },
    #     {
    #         "params": [
    #             p for n, p in model_without_ddp.named_parameters()
    #             if match_name_keywords(n, args.lr_segmentation_mask_head_names) and p.requires_grad
    #         ],
    #         "lr": args.lr_segmentation_mask_head,
    #         # "weight_decay": args.mask_head_weight_decay, 
    #     }
    # ]
    # ##################################################################################
