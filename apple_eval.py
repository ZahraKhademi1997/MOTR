...
def load_label(label_path: str, img_size: tuple) -> dict:
    labels=[]
    try:
        # labels0 = np.loadtxt(label_path, dtype=np.float32, delimiter=',').reshape(-1, 6)
        with open(label_path, 'r') as file:
            for line in file:
                parts = line.strip().split()  
                if len(parts) == 6:
                    # Convert frame ID and object ID to integers, and the rest to floats
                    frame_id, obj_id = map(int, parts[:2])
                    coordinates = map(float, parts[2:])
                    labels.append([frame_id, obj_id] + list(coordinates))
        labels0 = np.array(labels, dtype=np.float32)
...
# def load_label(mask_path: str, img_size: tuple) -> dict:
#     h, w = img_size
#     targets = {'boxes': [], 'labels': [], 'area': [], 'masks': [], 'iscrowd': [], 'obj_ids': []}
...
#             # Add mask and bbox to targets
#             targets['masks'].append(mask)
#             targets['boxes'].append(bbox)
#             targets['area'].append((xmax - xmin) * (ymax - ymin))
#             targets['iscrowd'].append(0)
#             targets['labels'].append(0)  # Update this if you have label information
#             targets['obj_ids'].append(object_id)

#     # Convert lists to numpy arrays
#     for key in ['boxes', 'area', 'labels', 'iscrowd', 'obj_ids']:
#         targets[key] = np.asarray(targets[key])
