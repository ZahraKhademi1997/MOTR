        ...
        for label in label_data:
            ...
            with open(mask_path, 'r') as f:
                for line in f:
                    ...
                    m_fid, m_obj_id, m_h, m_w, rle_str = int(parts[0]), int(parts[1]), int(parts[2]), int(parts[3])), parts[5]
                    # Check if the frame and object id from mask file match with label file
                    # if frame_id == m_fid and obj_id_orig == m_obj_id:
                    if frame_id == m_fid and obj_id == m_obj_id:
                        print('entered')
...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'AppleMOTS/train/images', self.seq_num, 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'AppleMOTS/train/images', self.seq_num, 'img1', _) for _ in img_list if
    ...
    def load_img_from_file(self, f_path):
        ...
        # mask_path = f_path.replace('images', 'instances')
        mask_path = f_path.replace('images', 'masks_with_ids').replace('.png', '.txt').replace('.jpg', '.txt')
        cur_img = cv2.imread(f_path)
        cur_img = cv2.cvtColor(cur_img, cv2.COLOR_BGR2RGB)
        targets = load_label(label_path, mask_path, cur_img.shape[:2]) if os.path.exists(label_path) else None
    ...
    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'AppleMOTS/train/images')
