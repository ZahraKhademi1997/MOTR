...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage=True, two_stage_num_proposals=900, num_queries_one2one = 100, num_queries_one2many = 900, decoder_self_cross=True, sigmoid_attn=False,
                 extra_track_attn=False, dn = True, dn_num=100, noise_scale=0.4, num_classes = 1, initial_pred=True, 
                 learn_tgt = True, initialize_box_type = False, query_dim = 4, dec_layer_share = False, 
                 with_box_refine=True, memory_bank=False):
        super().__init__()
        ...
        decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          ...
                                                          sigmoid_attn=sigmoid_attn, extra_track_attn=extra_track_attn, memory_bank=memory_bank)
        ...
        self.pos_det = None
        self.trk_embed = nn.Embedding(1, d_model)
        self.position = nn.Embedding(self.two_stage_num_proposals, 4)
        self.query_embed = nn.Embedding(self.two_stage_num_proposals, d_model)
    ...
    def dn_post_process(self,outputs_class,outputs_coord,mask_dict,outputs_mask, ref_points, hs):
        """
        ...
        return outputs_class, outputs_coord, outputs_mask, ref_points, hs, mask_dict
    ...
    def forward(self, srcs, masks, pos_embeds, embeddings, attention_embedding, targets = None, query_embed=None,  ref_pts=None, mem_bank=None, mem_bank_pad_mask=None):
        ...
        if self.two_stage:
            ...
            topk = self.two_stage_num_proposals + self.num_queries_one2many
            ...
            pos_det = reference_points_undetach[:, 0:self.num_queries_one2one, :].detach() # xywh format
            content_det = tgt_undetach[:, 0:self.num_queries_one2one, :].detach() # preventing gradient flow from decoder to encoder 
        ...
        if self.content_det is not None and self.pos_det is not None: 
            ...
            # tgt = content_det
            # reference_points = pos_det
            tgt = torch.cat([self.query_embed.weight, content_det.squeeze(0) + self.trk_embed.weight]).unsqueeze(0).expand(bs, -1, -1)
            reference_points = torch.cat([self.position.weight, pos_det.squeeze(0)]).unsqueeze(0).expand(bs, -1, -1)
        
        self.content_det = content_det.squeeze(0)
        self.pos_det = pos_det.squeeze(0)
        
        # Adding one2many queries
        # if self.training and self.content_det is not None and self.pos_det is not None:
        #     tgt=torch.cat([tgt_undetach[:, len(self.content_det) :, :].detach(), tgt],dim=1)
        #     reference_points=torch.cat([reference_points_undetach[:, len(self.content_det) :, :].detach(), reference_points],dim=1)
        # if self.training and self.content_det is None and self.pos_det is None:
        #     tgt=torch.cat([tgt_undetach[:, self.num_queries_one2one :, :].detach(), tgt],dim=1)
        #     reference_points=torch.cat([reference_points_undetach[:, self.num_queries_one2one :, :].detach(), reference_points],dim=1)
        ...
        # Two separate the queries for one2one and one2many 
        # index = reference_points.shape[1] - self.num_queries_one2many    
        # self_attn_mask = (torch.zeros([reference_points.shape[1], reference_points.shape[1],]).bool().to(src_flatten.device))
        # self_attn_mask[index :, 0 : index,] = True
        # self_attn_mask[0 : index, index :,] = True
        # print('self_attn_mask:', self_attn_mask.shape)
        
        if self.training and self.content_det is not None and self.pos_det is not None:
            tgt_one2many = tgt_undetach[:, len(self.content_det) :, :].detach()
            reference_points_one2many = reference_points_undetach[:, len(self.content_det) :, :].detach()
        if self.training and self.content_det is None and self.pos_det is None:
            tgt_one2many = tgt_undetach[:, self.num_queries_one2one :, :].detach()
            reference_points_one2many = reference_points_undetach[:, self.num_queries_one2one :, :].detach()
        
        init_reference_out = torch.cat([reference_points_one2many, reference_points],dim=1) 

        # Queries: Detect queries (query selection) + track queries (QIM) + denoising queries (DN)
        hs_one2one_list, inter_references_one2one_list = self.decoder(
            ...
            mem_bank = mem_bank, 
            mem_bank_pad_mask = mem_bank_pad_mask,
            # pad_noise_size = None,
            memory_padding_mask=mask_flatten,
            self_attn_mask = None,
        ) 
         
        hs_one2many_list, inter_references_one2many_list = self.decoder(
            tgt=tgt_one2many.transpose(0, 1),  
            memory=memory.transpose(0, 1),
            # memory_mask=None,
            # tgt_key_padding_mask=None,
            pos=None,
            refpoints_unsigmoid=reference_points_one2many.transpose(0,1),
            level_start_index=level_start_index,
            spatial_shapes=spatial_shapes,
            valid_ratios=valid_ratios,
            mem_bank = mem_bank, 
        ...
        # for idx, tensor in enumerate(hs_one2one_list):
        #     print(f"hs[{idx}]: {tensor.shape}")
        hs_one2one = torch.stack(hs_one2one_list, dim=0)
        hs_one2many = torch.stack(hs_one2many_list, dim=0)
        inter_references_one2one = torch.stack(inter_references_one2one_list, dim=0)
        inter_references_one2many = torch.stack(inter_references_one2many_list, dim=0)
        hs = torch.cat([hs_one2one, hs_one2many], dim=2)
        inter_references = torch.cat([inter_references_one2one, inter_references_one2many], dim=2)
        
        inter_references_out = inter_references
        if self.two_stage:
            return hs, init_reference_out, inter_references_out, interm_outputs, mask_dict
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 ...
                 extra_track_attn=False, use_deformable_box_attn=False,
                 memory_bank=False, key_aware_type=None, ):
        super().__init__()
    ...
    def _forward_self_attn(self, tgt, query_pos, self_attn_mask=None):
        if self.extra_track_attn:
            tgt = self._forward_track_attn(tgt, query_pos)
        q = k = self.with_pos_embed(tgt, query_pos)
        if self_attn_mask is not None:
            print("tgt.transpose(0, 1) shape:", tgt.transpose(0, 1).shape, "query_pos:", query_pos.shape, 
                  "q.transpose(0, 1) shape:", q.transpose(0, 1).shape,"k.transpose(0, 1) shape:", k.transpose(0, 1).shape,
    ...
    def _forward_track_attn(self, tgt, query_pos):
        q = k = self.with_pos_embed(tgt, query_pos)
        if q.shape[1] > 120:
            tgt2 = self.update_attn(q[:, 120:].transpose(0, 1),
                                    k[:, 120:].transpose(0, 1),
                                    tgt[:, 120:].transpose(0, 1))[0].transpose(0, 1)
            tgt = torch.cat([tgt[:, :120],self.norm4(tgt[:, 120:]+self.dropout5(tgt2))], dim=1)
        return tgt


    def _forward_self_cross(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index,
                             memory_padding_mask=None, self_attn_mask=None):
        ...
        # self attention
        tgt = self._forward_self_attn(tgt, query_pos, self_attn_mask)
        # cross attention
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, query_pos).transpose(0, 1),
                               reference_points.transpose(0, 1).contiguous(),
                               src.transpose(0, 1), src_spatial_shapes, level_start_index, memory_padding_mask).transpose(0, 1)
    ...
    def forward(self,
        ...
        #                                     memory_level_start_index, memory_key_padding_mask, attn_masks[attn_index])
        # return self._forward_cross_self(tgt, tgt_query_pos, tgt_reference_points, memory, memory_spatial_shapes,
        #                                     memory_level_start_index, memory_key_padding_mask, attn_masks[attn_index])
        if self.self_cross:
            return self._forward_self_cross(tgt, tgt_query_pos, tgt_reference_points, memory, memory_spatial_shapes,
                                            memory_level_start_index, memory_padding_mask, self_attn_mask)
        return self._forward_cross_self(tgt, tgt_query_pos, tgt_reference_points, memory, memory_spatial_shapes,
                                            memory_level_start_index, memory_padding_mask, self_attn_mask)
