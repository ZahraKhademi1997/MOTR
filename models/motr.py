...
class ClipMatcher(SetCriterion):
    ...
    # (2) Adding loss_mask from deformable_detr to motr barrowed from @https://bowenc0221.github.io/mask2former/ 
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        def plot_and_save_masks(active_idxes, predicted_masks, ground_truth_masks, output_dir):
            ...
            for i in range(num_plots):
                ...
                if i < num_active:
                    axes[0].imshow(active_predictions[i].detach().cpu().numpy(), cmap='gray')
                    axes[0].set_title(f"Active Pred Mask {i+1}")
                    axes[0].axis('off')
                if i < num_gt:
                    axes[1].imshow(ground_truth_masks[i].detach().cpu().numpy(), cmap='gray')
        ...
        if len(active_track_masks_primarly) > 0: 
            ...
            plot_and_save_masks(active_idxes, track_instances.pred_masks, gt_masks, '/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-AppleMOTS-Axial_Cross_Attention/output/criterion')
