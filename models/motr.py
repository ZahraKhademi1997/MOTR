...
class ClipMatcher(SetCriterion):
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        def save_masks_side_by_side(output_dir, pred_masks, gt_masks, active_idxes):
            ...
            for idx in active_idxes:
                ...
                # Squeeze masks in case they have an extra dimension
                pred_mask = pred_masks[idx].squeeze().cpu().numpy()
                gt_mask = gt_masks[idx].squeeze().cpu().numpy()
                
                axs[0].imshow(pred_mask, cmap='gray')
                axs[0].set_title(f'Predicted Mask Index {idx}')
                axs[0].axis('off')

                axs[1].imshow(gt_mask, cmap='gray')
                axs[1].set_title(f'Ground Truth Mask Index {idx}')
                axs[1].axis('off')

                plt.tight_layout()
                plt.savefig(output_dir / f'mask_comparison_index_{idx}.png')
                plt.close() 
        ...
        #     # Calculate the intersection (element-wise minimum)
        #     intersection = torch.min(active_track_masks_primarly, gt_masks)
        #     # Calculate the union (element-wise maximum)
        #     union = torch.max(active_track_masks_primarly, gt_masks)
        #     # Calculate soft IoU weights; this will be in the range [0, 1]
        #     iou_weights = intersection / (union + 1e-6)  
        #     print('iou_weights:', iou_weights)
        #     iou_threshold = 0.1
        #     significant_overlap = iou_weights > iou_threshold
        #     active_track_masks = active_track_masks_primarly * significant_overlap.float()
        #     print('active_track_masks shape:', active_track_masks.shape)
        #     print('active_tracck_masks:', active_track_masks)
        #     save_masks_side_by_side('/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR-mask-AppleMOTS-MaskFormer/output/matcher', active_track_masks, gt_masks, active_idxes)
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        self.AxialBlock = AxialBlock(256,128)
    ...
    def _forward_single_image(self, samples, track_instances: Instances):
        ...
        # print('similarity_h:', similarity_h.shape, 'similarity_w:', similarity_w.shape)
        # print('attention_embedding:', attention_embedding) --> all positive values
        ...
        # pred_masks = torch.einsum('bqc,bchw->bqhw',  mask_embeddings_prob, attention_embedding)
        # print('pred_masks shape:', pred_masks.shape) # torch.Size([1, 300, 92, 144])
        # pred_masks = F.relu(pred_masks)
        # print('pred_masks:', pred_masks)

        predicted_masks = []
        for i in range(mask_embeddings_prob.shape[1]):
        ...
        # print('pred_mask:', pred_masks.shape)
