...
from util.DN import prepare_targets,repeat_GT_instances
import os
...
class ClipMatcher(SetCriterion):
    ...
    def loss_labels(self, outputs, gt_instances: List[Instances], indices, num_boxes, log=False):
        ...
        if self.focal_loss:
            ...
            loss_ce = sigmoid_focal_loss(src_logits.flatten(1),
                                             ...
                                             alpha=0.25, # First experiment with alpha=-1, second with alpha=0.25, third with alpha=0.75, fourth with alpha=-1
                                             gamma=2, # First experiment with gamma=0, second with gamma=2, third with gamma=2, fourth with gamma=1
    ...
    def loss_masks(self, outputs, gt_instances: List[Instances], indices, num_boxes):
        ...
        #     target_masks, valid = nested_tensor_from_tensor_list(new_masks).decompose()
        ########################################################################################
        def save_sampled_points_on_masks(src_mask, tgt_mask, uncertain_point_coords, random_point_coords, index=0, output_dir="/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-hybrid_matching/output/point_render"):
            # Setup plot
            fig, axs = plt.subplots(1, 2, figsize=(12, 6))
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        outputs_without_aux = {k: v for k, v in outputs.items() if k != 'aux_outputs' and k != 'interm_outputs' and k != 'outputs_one2many' and k != 'aux_outputs_one2many'}
        ...
        def one2many_match_for_single_decoder_layer(unmatched_outputs, matcher):
            # untracked_gt_instances = repeat_GT_instances (untracked_gt_instances, n_repeats=6)
            new_track_indices = matcher(unmatched_outputs,
                                             [repeat_GT_instances(gt_instances_i, n_repeats=6)])  # list[tuple(src_idx, tgt_idx)]
            src_idx, tgt_idx = new_track_indices[0]
            indices =torch.stack([src_idx, tgt_idx], dim=1).to(unmatched_outputs['pred_logits'].device)
            return indices
        
        def interm_match_for_single_decoder_layer(unmatched_outputs, matcher):
            # untracked_gt_instances = repeat_GT_instances (untracked_gt_instances, n_repeats=6)
            new_track_indices = matcher(unmatched_outputs,
        ...
        if len(active_track_masks_primarly) > 0: 
            ...
            plot_and_save_masks(active_idxes, track_instances.pred_masks, gt_masks, gt_boxes, active_track_boxes, '/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-hybrid_matching/output/criterion')
        ...
        if 'interm_outputs' in outputs:
            ...
            interm_matched_indices_layer = interm_match_for_single_decoder_layer(unmatched_outputs_layer_interm, self.matcher)
            for loss in self.losses:
                l_dict = self.get_loss(loss,
                                       interm_outputs,
                                       gt_instances=[gt_instances_i],
        ...
        if 'outputs_one2many' in outputs:
            # print("Full pred_boxes shape:", outputs['interm_outputs']['pred_boxes'].shape)
            one2many_outputs = outputs['outputs_one2many']
            unmatched_outputs_layer_one2many = {
                    'pred_logits': one2many_outputs['pred_logits'],
                    'pred_boxes': one2many_outputs['pred_boxes'],
                    'pred_masks': one2many_outputs['pred_masks'],
                }
            one2many_matched_indices_layer = one2many_match_for_single_decoder_layer(unmatched_outputs_layer_one2many, self.matcher)
            
            for loss in self.losses:
                l_dict = self.get_loss(loss,
                                       one2many_outputs,
                                       gt_instances=[repeat_GT_instances(gt_instances_i, n_repeats=6)],
                                       indices=[(one2many_matched_indices_layer[:, 0], one2many_matched_indices_layer[:, 1])],
                                       num_boxes=1)
                l_dict = {k + f'_one2many': v for k, v in l_dict.items()}
                self.losses_dict.update(
                    {'frame_{}_{}'.format(self._current_frame_idx, key): value for key, value in
                    l_dict.items()})
        
        if 'aux_outputs_one2many' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs_one2many']):    
                unmatched_outputs_layer_aux_one2many = {
                        'pred_logits': aux_outputs['pred_logits'],
                        'pred_boxes': aux_outputs['pred_boxes'],
                        'pred_masks': aux_outputs['pred_masks'],
                    }
                one2many_aux_matched_indices_layer = one2many_match_for_single_decoder_layer(unmatched_outputs_layer_aux_one2many, self.matcher)
                
                for loss in self.losses:
                    l_dict = self.get_loss(loss,
                                        aux_outputs,
                                        gt_instances=[repeat_GT_instances(gt_instances_i, n_repeats=6)],
                                        indices=[(one2many_aux_matched_indices_layer[:, 0], one2many_aux_matched_indices_layer[:, 1])],
                                        num_boxes=1)
                    l_dict = {k + f'_one2many': v for k, v in l_dict.items()}
                    self.losses_dict.update(
                        {'frame_{}_aux{}_{}'.format(self._current_frame_idx, i, key): value for key, value in
...
# class RuntimeTrackerBase(object):
#     def __init__(self, score_thresh=0.3, filter_score_thresh=0.8, miss_tolerance=5):
#         self.score_thresh = score_thresh
#         self.filter_score_thresh = filter_score_thresh
#         self.miss_tolerance = miss_tolerance
#         self.max_obj_id = 0
...
class RuntimeTrackerBase(object):
    def __init__(self, score_thresh=0.4, filter_score_thresh=0.7, miss_tolerance=5):
        self.score_thresh = score_thresh
        self.filter_score_thresh = filter_score_thresh
        self.miss_tolerance = miss_tolerance
        self.max_obj_id = 0
    ...
    def update(self, track_instances: Instances):
        ...
        # Reset disappear time for detections above score threshold
        disappear_time[scores >= self.score_thresh] = 0

        # Convert from cxcywh to xyxy for NMS
        boxes_xyxy = box_cxcywh_to_xyxy(boxes)

        # Apply NMS to all detections with scores above the score threshold
        keep = nms(boxes_xyxy, scores, iou_threshold=0.1)  # You can adjust the IoU threshold

        # Process NMS results: update existing tracks or create new tracks
        for idx in keep:
            if obj_idxes[idx] == -1 and scores[idx] >= self.score_thresh:
                # Assign a new ID to detections not already tracked
                print(f"Assigning new ID {self.max_obj_id} to detection {idx}")
                obj_idxes[idx] = self.max_obj_id
                self.max_obj_id += 1
            elif obj_idxes[idx] >= 0:
                if scores[idx] < self.filter_score_thresh:
                    disappear_time[idx] += 1
                    if disappear_time[idx] >= self.miss_tolerance:
                        # Set the obj_id to -1, marking it for removal
                        obj_idxes[idx] = -1

        # Optional: Remove all non-kept detections or reset their ids based on policy
        # This step depends on how you want to handle detections that weren't kept by NMS
        for i in range(len(scores)):
            if i not in keep and obj_idxes[i] != -1:
                # Policy decision: reset ID for non-kept but previously tracked objects
                obj_idxes[i] = -1
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_queries_one2one, num_queries_one2many, num_feature_levels, criterion, track_embed, num_seg_fcs, mask_positional_encoding_cfg={'type': 'RelSinePositionalEncoding', 'num_feats': 128, 'normalize': True},
    ...
    def _forward_single_image(self, samples, targets, track_instances: Instances):
        ...
        hs, init_reference, inter_references, interm_outputs, mask_dict= self.transformer(srcs, masks, pos, embeddings, attention_embedding, targets, track_instances.query_pos,track_instances.ref_pts, mem_bank=track_instances.mem_bank, mem_bank_pad_mask=track_instances.mem_padding_mask)
        ...
        outputs_class_one2one = []
        outputs_coord_one2one = []
        outputs_class_one2many = []
        outputs_coord_one2many = []
        outputs_masks_one2many = []
        outputs_masks_one2one = []
        ref_pts_all_one2many = []
        ref_pts_all_one2one = []
        hs_one2one = []
        hs_one2many = []
        ...
        if mask_dict is not None:
            outputs_class, outputs_coord, pred_masks, ref_pts_all, hs, mask_dict = self.dn_post_process(outputs_class, outputs_coord, mask_dict, pred_masks, ref_pts_all, hs)
            # outputs_class, outputs_coord, pred_masks, ref_pts_all, hs = list(outputs_class), list(outputs_coord), list(pred_masks), list(ref_pts_all), list(hs)
            outputs_class = torch.tensor(outputs_class) if isinstance(outputs_class, list) else outputs_class
            outputs_coord = torch.tensor(outputs_coord) if isinstance(outputs_coord, list) else outputs_coord
            pred_masks = torch.tensor(pred_masks) if isinstance(pred_masks, list) else pred_masks
            ref_pts_all = torch.tensor(ref_pts_all) if isinstance(ref_pts_all, list) else ref_pts_all
            hs = torch.tensor(hs) if isinstance(hs, list) else hs
            
        index = hs.shape[2] - self.num_queries_one2many
        outputs_class_one2one.append(outputs_class[:, :, : index, :])
        outputs_class_one2many.append(outputs_class[:, :, index :, :])
        outputs_coord_one2one.append(outputs_coord[:, :, : index, :])
        outputs_coord_one2many.append(outputs_coord[:, :, index :, :])
        outputs_masks_one2one.append(pred_masks[:, :, : index, :, :])
        outputs_masks_one2many.append(pred_masks[:, :, index :, :, :])
        hs_one2one.append(hs[:, :, : index, :])
        hs_one2many.append(hs[:, :, index :, :])
        ref_pts_all_one2one.append(ref_pts_all[:, :, : index, :])
        ref_pts_all_one2many.append(ref_pts_all[:, :, index :, :])
        
        outputs_class_one2one = torch.stack(outputs_class_one2one).squeeze(0)
        outputs_coord_one2one = torch.stack(outputs_coord_one2one).squeeze(0)
        outputs_class_one2many = torch.stack(outputs_class_one2many).squeeze(0)
        outputs_coord_one2many = torch.stack(outputs_coord_one2many).squeeze(0)
        outputs_masks_one2many = torch.stack(outputs_masks_one2many).squeeze(0)
        outputs_masks_one2one = torch.stack(outputs_masks_one2one).squeeze(0)
        ref_pts_all_one2one = torch.stack(ref_pts_all_one2one).squeeze(0)
        ref_pts_all_one2many = torch.stack(ref_pts_all_one2many).squeeze(0)
        hs_one2one = torch.stack(hs_one2one).squeeze(0)
        hs_one2many = torch.stack(hs_one2many).squeeze(0)
        # print("Outputs class one2one shape:", outputs_class_one2one.shape,
        #       "Outputs class one2many shape:", outputs_class_one2many.shape,
        #       "Outputs masks one2one shape:", outputs_masks_one2one.shape,
        #       "Outputs masks one2many shape:", outputs_masks_one2many.shape,
        #       "Outputs coord one2one shape:",  outputs_coord_one2one.shape, 
        #       "Outputs coord one2many shape:", outputs_coord_one2many.shape, 
        #       "Ref pts all one2one shape:", ref_pts_all_one2one.shape,
        #       "Ref pts all one2many shape:", ref_pts_all_one2many.shape, 
        #       "HS one2one shape:", hs_one2one.shape,
        #       "HS one2many shape:", hs_one2many.shape)
        
        out = {'pred_logits': outputs_class_one2one[-1], 'pred_boxes': outputs_coord_one2one[-1], 
               'ref_pts': ref_pts_all_one2one[5], 'pred_masks': F.interpolate(
                outputs_masks_one2one[-1],
        ...
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class_one2one, outputs_masks_one2one, outputs_coord_one2one)
            out['aux_outputs_one2many'] = self._set_aux_loss(outputs_class_one2many, outputs_masks_one2many, outputs_coord_one2many)
        
        out['hs'] = hs_one2one[-1]
        out['hs_one2many'] = hs_one2many[-1]
        
        out['interm_outputs'] = interm_outputs # query selection from encoder
        out['outputs_one2many'] = {'pred_logits': outputs_class_one2many[-1], 
                'pred_boxes': outputs_coord_one2many[-1], 'ref_pts': ref_pts_all_one2many[5], 
                'pred_masks': F.interpolate(
                outputs_masks_one2many[-1],
                size=(samples.tensors.shape[2], samples.tensors.shape[3]),
                mode="bilinear",
                align_corners=False),
    ...
    def _post_process_single_image(self, frame_res, track_instances, frame_shape, is_last):             
        ...
        # track_instances.pred_logits_one2many = frame_res['pred_logits_one2many'][0]
        # track_instances.pred_boxes_one2many = frame_res['pred_boxes_one2many'][0]
        # track_instances.pred_masks_one2many = frame_res['pred_masks_one2many'][0]
    ...
    def forward(self, data: dict):
        ...
        for frame_index, (frame, target) in enumerate(zip(frames, targets)):
            ...
            if len(track_instances) > (2*self.num_queries_one2one):
                frame_res = self._post_process_single_image(frame_res, track_instances, frame_shape,is_last)
            else: 
                track_instances = self._generate_empty_tracks(frame_shape, device)
                frame_res = self._post_process_single_image(frame_res, track_instances, frame_shape,is_last)
...
def build(args):
    ...
    for i in range(num_frames_per_batch):
        weight_dict.update({"frame_{}_loss_ce".format(i): args.cls_loss_coef,
                            ...
                            "frame_{}_loss_ce_one2many".format(i): args.cls_loss_coef,
                            'frame_{}_loss_bbox_one2many'.format(i): args.bbox_loss_coef,
                            'frame_{}_loss_giou_one2many'.format(i): args.giou_loss_coef,
                            'frame_{}_loss_mask_one2many'.format(i): args.mask_loss_coef,
                            'frame_{}_loss_dice_one2many'.format(i): args.dice_loss_coef,
        ...
        for i in range(num_frames_per_batch):
            for j in range(args.dec_layers - 1):
                weight_dict.update({"frame_{}_aux{}_loss_ce".format(i, j): args.cls_loss_coef,
                                    ...
                                    "frame_{}_aux{}_loss_ce_one2many".format(i, j): args.cls_loss_coef,
                                    'frame_{}_aux{}_loss_bbox_one2many'.format(i, j): args.bbox_loss_coef,
                                    'frame_{}_aux{}_loss_giou_one2many'.format(i, j): args.giou_loss_coef,
                                    'frame_{}_aux{}_loss_mask_one2many'.format(i, j): args.mask_loss_coef,
                                    'frame_{}_aux{}_loss_dice_one2many'.format(i, j): args.dice_loss_coef,
                                    })
    ...
    num_queries_one2one = args.num_queries_one2many
    num_queries = args.num_queries

    criterion = ClipMatcher(num_classes, matcher=img_matcher, weight_dict=weight_dict, losses=losses, num_points = num_points, oversample_ratio = oversample_ratio, importance_sample_ratio = importance_sample_ratio ,num_queries_one2one = num_queries_one2one, num_queries = num_queries, dn_losses = dn_losses, dn = dn)
    criterion.to(device)
    postprocessors = {}
    model = MOTR(
        backbone,
