...
import json
from util.box_ops import (clamp_boxes, masks_to_boxes, 
                          normalize_boxes, box_cxcywh_to_xyxy, 
                          box_xyxy_to_cxcywh, box_iou,
                          cxcywh_to_tlbr, cxcyah_to_tlbr)
...
from util.probabilistic_association.track_association import Association, TrackState
from util.probabilistic_association.association_methods import iou_distance, linear_assignment
from util.probabilistic_association.kalman_filter import KalmanFilter
from util.box_ops import box_cxcywh_to_cxcyah, box_cxcyah_to_cxcywh
import cv2
import numpy as np
import random
...
#     def __init__(self, score_thresh=0.2, filter_score_thresh=0.7, miss_tolerance=7):
...
#         keep = nms(boxes_xyxy, scores, iou_threshold=0.5)  # You can adjust the IoU threshold
...
class RuntimeTrackerBase(object):
    ...
    def activate(self, detection):
        mean, covariance = self.kf.initiate(detection)
        self.next_id()
        self.tracklet_len = 0
        self.state = TrackState.New
        self.is_activated = True
        return mean, covariance, self.max_obj_id
    
    def re_activate(self, new_track, mean, covariance, frame_id, new_id = False):
        mean, covariance = self.kf.update(
            mean, covariance, box_cxcywh_to_cxcyah(new_track))
        ...
        #     return mean, covariance, track_id
    ...
    def multi_predict(self, tracks):
        if len(tracks) > 0:
            ...
            multi_mean, multi_covariance = self.kf.multi_predict(multi_mean, multi_covariance)
            for i, (mean, cov) in enumerate(zip(multi_mean, multi_covariance)):
                tracks[i]['mean'] = mean
                tracks[i]['covariance'] = cov
                
    def tracklet_update(self, track, mean, covariance, frame_id):
        self.frame_id = frame_id
        self.tracklet_len += 1
        mean, covariance = self.kf.update(
            mean, covariance, box_cxcywh_to_cxcyah(track))
        self.state = TrackState.Tracked
        self.is_activated = True
        return mean, covariance
    
    def save_tracks_to_file(self):
        # Define the file path where you want to save all frames' data
        file_path = "/home/zahra/Documents/Projects/prototype/MOTR-codes/MOTR_KF/output/kf/seq02.txt"

        # Prepare the data to be written for the current frame
        data_to_write = {
        ...
        for track in self.tracks:
            track_data = {
                ...
                "mean": track['mean'].tolist(),  # Assuming mean is a numpy array or similar
                "covariance": track['covariance'].tolist()  # Assuming covariance is a numpy array or similar
            }
            data_to_write["tracks"].append(track_data)

        # Write the data to file in append mode
        with open(file_path, 'a') as file:  # 'a' mode appends to the file without truncating it
            file.write(json.dumps(data_to_write) + '\n')  # Write each frame's data in a new line

        print(f"Data for frame {self.frame_id} saved.")
    ...
    def update(self, track_instances: Instances):
        ...
        for idx in keep:
            if scores[idx] >= self.score_thresh: 
                keep_threshold.append(idx.item())
            if obj_idxes[idx] == -1 and scores[idx] >= self.score_thresh:
                keep_ids.append(idx.item())
            
        keeps = torch.tensor(keep_threshold, dtype=torch.long).to(device)
        keeps_ids = torch.tensor(keep_ids, dtype=torch.long).to(device)
        valid_boxes = boxes[keeps] # cxcywh
        valid_scores = scores[keeps] 
        print('valid_boxes:', valid_boxes.shape)

        # Estimation based on the current tracks
        self.multi_predict(self.tracks)
        
        # Initialize the tracker
        if not self.tracks:   
            for det in valid_boxes:
                init_mean, init_covariance, track_id = self.activate(box_cxcywh_to_cxcyah(det)) # cxcyah and numpy.ndarray
                # Only in the first frame of each sequence
                self.tracks.append({'mean': init_mean, 'covariance': init_covariance, 'track_id': track_id})

        # Associate the detections from prev frame with detections in the current frame
        dists = iou_distance(cxcyah_to_tlbr([track['mean'][:4] for track in self.tracks]), cxcywh_to_tlbr(valid_boxes)) # Similarity matching
        matches, u_track, u_detection = linear_assignment(dists, thresh=self.match_thresh)
        print('u_track:', u_track, 'u_detection:', u_detection, 'matches:', matches) # u_tracks: detection in the previous frame that did not match, u_detections: detection in the current frame that did not match

        # Update the tracker with the matched detections
        for itracked, idet in matches:
            det = valid_boxes[idet]
            update_mean, updated_covariance = self.tracklet_update(det.cpu().numpy(), self.tracks[itracked]['mean'], self.tracks[itracked]['covariance'], self.frame_id)
            self.tracks[itracked]['mean'] = update_mean
            self.tracks[itracked]['covariance'] = updated_covariance
                        
        # Save lost tracks to count for occlusions  
        for it in u_track:
            self.activated = False
            self.lost_frame += 1
            self.lost_tracks.append({'mean': self.tracks[it]['mean'], 'covariance' : self.tracks[it]['covariance'], 'original_index': it})
            # Remove the track from the active list
            # self.tracks.pop(it)

        # Match new detections to occluded objects or assign new ids
        if len(u_detection) > 0:
            new_det = torch.stack([valid_boxes[inew] for inew in u_detection])
            
            if self.lost_frame < 5:
                lost_means = [lost_track['mean'] for lost_track in self.lost_tracks]
                lost_covariances = [lost_track['covariance'] for lost_track in self.lost_tracks]
                lost_bboxes = [lost_track['mean'][:4] for lost_track in self.lost_tracks]
                original_indices = [lost_track['original_index'] for lost_track in self.lost_tracks]
                
                dists_tolerance = iou_distance(cxcyah_to_tlbr(lost_bboxes), cxcywh_to_tlbr(new_det)) # Similarity matching
                matches_tolerance, u_track_tolerance, u_detection_tolerance = linear_assignment(dists_tolerance, thresh=0.7)
                print('matches_tolerance:', matches_tolerance, 'u_track_tolerance:', u_track_tolerance, 'u_detection_tolerance:', u_detection_tolerance)

                if len(matches_tolerance) > 0:
                    # If using keep instead of keep_threshold
                    if not isinstance(matches_tolerance, list):
                        matches_tolerance = matches_tolerance.tolist()  

                    matches_tolerance.sort(key=lambda x: x[0], reverse=True)

                    for itracked_t, idet_t in matches_tolerance:
                        det_t = new_det[idet_t]
                        mean_t, covariance_t = self.re_activate(det_t, lost_means[itracked_t], lost_covariances[itracked_t], self.frame_id, new_id=False)
                        
                        # Reactivate the track with new details
                        index = original_indices[itracked_t]
                        self.tracks[index]['mean'] = mean_t
                        self.tracks[index]['covariance'] = covariance_t
                        
                        # Remove the matched lost track from the list
                        self.lost_tracks.pop(itracked_t)

                if len(u_detection_tolerance) > 0:
                    for inew_t in u_detection_tolerance:
                        new_det_t = new_det[inew_t]
                        mean_n_t, covariance_n_t, track_id_n_t = self.activate(box_cxcywh_to_cxcyah(new_det_t))
                        self.tracks.append({'mean': mean_n_t, 'covariance': covariance_n_t, 'track_id': track_id_n_t})
                       
            else:
                self.reset_lost()
                for inew in u_detection:
                    new_det = valid_boxes[inew]
                    mean_new, covariance_new, track_id_new = self.activate(box_cxcywh_to_cxcyah(new_det))
                    self.tracks.append({'mean': mean_new, 'covariance': covariance_new, 'track_id': track_id_new})
                    
        # ID assignment
        track_ids = [track['track_id'] for track in self.tracks]
        track_id_map = {keep_threshold[i]: track_ids[i] for i in range(len(keep_threshold))}
        print('track_id_map:', track_id_map)
        for idx in keep_threshold:
            if idx in track_id_map:  
                if track_id_map[idx] not in obj_idxes and obj_idxes[idx] == -1:
                    obj_idxes[idx] = track_id_map[idx]
                # else:
                    # make sure the existance object id and track ids are correspondace
                # if obj_idxes[idx] not in track_id_map.values():
                #     obj_idxes[idx] = -1
...
class MOTR(nn.Module):
    ...
    def _forward_single_image(self, samples, track_instances: Instances, targets=None):
        features, pos = self.backbone(samples)
        src, mask = features[-1].decompose()
        
        img_height, img_width = samples.tensors.shape[2], samples.tensors.shape[3]
        ...
        for lvl in range(hs.shape[0]):
            ...
            # outputs_coords.append(clamp_boxes(outputs_coord, img_width, img_height))
            outputs_coords.append(outputs_coord)
            outputs_dynamic_params.append(dynamic_params) # Query embedding
        outputs_class = torch.stack(outputs_classes)
        outputs_coord = torch.stack(outputs_coords) # cx, cy, w, h
