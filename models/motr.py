...
#####################################################################################################################
# (1) Adding functions from segmentation function
...
class ClipMatcher(SetCriterion):
    def __init__(self, num_classes,
        ...
        ##################################################################
        # (2) Adding some notations for the forward function
        # TODO make it adjustable for batch size more than 1
    ...
    def get_loss(self, loss, outputs, gt_instances, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            ######################################
            # (3) Adding loss_masks to dictionary
    ...
    def loss_boxes(self, outputs, gt_instances: List[Instances], indices: List[tuple], num_boxes):
        ...
        losses['loss_giou_bbox'] = loss_giou.sum() / num_boxes
    ...
    def loss_labels(self, outputs, gt_instances: List[Instances], indices, num_boxes, log=False):
        ...
        """
        # print('outputs in loss_label is:', outputs)
        ...
        # print('losses in loss_label is:', losses)
        return losses
    
    ############################################################################################################
    # (4) Adding loss_mask from deformable_detr to motr     
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        # print(' obj_idx_to_gt_idx in motr.py are:',  obj_idx_to_gt_idx) # {3000814.0: 0, 3000816.0: 1, 3000817.0: 2, 3000818.0: 3, 3000819.0: 4, 3000827.0: 5, 3000832.0: 6}
        outputs_i = {
        ...
        def match_for_single_decoder_layer(unmatched_outputs, matcher):
            ...
            # concat src and tgt.
            ########################################################################################################
            # (7) Solving device problem in qim
        ...
        if len(active_track_boxes) > 0:
            ...
            ###############################################################################
            # (10) Adding active track masks
            # TODO write iou calculation based on masks
            ...
            # print('active_track_masks in motr has the shape of:', active_track_masks.shape) #torch.Size([7, 864, 1536]) --> Binary 
            # print('gt_masks in motr has the shape of:', gt_masks.shape) #torch.Size([7, 864, 1536]) --> Boolian
            ...
            print(' track_instances.iou_masks in motr is:',  track_instances.iou_masks)
            # print(' track_instances.iou_boxes in motr is:',  track_instances.iou_boxes)
            ##############################################################################
...
class MOTR(nn.Module):
    ...
    def _generate_empty_tracks(self):
        ...
        #######################################################################################################
        # (15) Adding part to handle iou from masks and boxes and adding masks to instances
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord):
        ...
        # print(outputs_class[:-1].shape) # torch.Size([5, 1, 305, 1])
        # print(outputs_coord[:-1].shape) #torch.Size([5, 1, 305, 4])
        return [{'pred_logits': a, 'pred_boxes': b}
                for a, b in zip(outputs_class[:-1], outputs_coord[:-1])]
        
    ######################################################################################
    # (16) For training the pred_masks shuold be incorporated
    def _forward_single_image(self, samples, track_instances: Instances):
        ...
        # (25) Getting memory from the last layer of the backbone
        ...
        ##############################################################################
        # (19) Changing the memory shape to calculate masks
        bs, c, h, w = srcs[3].shape
        memory_mask = memory_mask.view(bs, c, h, w)
        bbox_mask = self.bbox_attention(hs[-1], memory_mask, mask=masks[3])
        # print('bbox_mask is:', bbox_mask.shape) # torch.Size([1, 300, 8, 10, 14])
        # print('bbox_mask in segmentation has the shape of:', bbox_mask.shape) #torch.Size([1, 310, 8, 14, 24])
        ...
        # print('pred_maskss has the shape of:', pred_maskss) # torch.Size([1, 300, 80, 111])
        
        
        ##############################################################
        # (20) Getting image dimension for the post processing purposes
        max_target_sizes = []
        ...
        # print('orig_target_sizes', orig_target_sizes) #  orig_target_sizes tensor([[ 80, 111]])
        # print('max_target_sizes', max_target_sizes) #  max_target_sizes tensor([[640, 888]])
    ...
    # TODO edit this to get the mask like previous functions
    @torch.no_grad()
    def inference_single_image(self, img, ori_img_size, track_instances=None):
        
        # print('track_instances in MOTR from submit are:', track_instances)
        if not isinstance(img, NestedTensor):
            img = nested_tensor_from_tensor_list(img)
           
        ##############################################################
        # () Getting image dimension for the post processing purposes
        max_target_sizes = []
        for img_tensor in img.tensors:
        ...
        ###################################################################################
        # () Calculating masks attribute for each image for _generate_empty_tracks function
        self.mask_height , self.mask_width = img.tensors.shape[-2:]
        ###################################################################################
        

        ################################################################################
        # () Predicting masks from the segmentation mask
        pred_maskss = self.segm_head(img, track_instances)['pred_masks']
        ################################################################################
        # () Resizing masks
        max_target_sizes = max_target_sizes
        max_target_sizes = torch.tensor(max_target_sizes)
        ...
        postprocessed_masks = postprocessor([], {'pred_masks': pred_maskss}, orig_target_sizes, max_target_sizes)
        # pred_masks = postprocessed_masks['masks'].to(torch.float32)
        pred_masks = postprocessed_masks.to(torch.float32)
        # pred_masks = pred_masks.permute(1 , 0 , 2 , 3)
        # print('pred_masks after postprocessing is:', pred_masks.shape) # torch.Size([1, 300, 864, 1536]), Binary
        ################################################################################################################################################
    ...
    def forward(self, data: dict):
        ...
        ###################################################################################
        # (23) Calculating masks attribute for each image for _generate_empty_tracks function
        # Batch size of 1
        self.mask_height , self.mask_width = frames[0].shape[-2:]
        # Batch size of 2
        # for frame in frames:
        #     mask_heights , mask_widths = frame[0].shape[-2:]
        #     self.mask_height.append(mask_heights)
        #     self.mask_width.append(mask_widths)
        ...
        for frame_index, frame in enumerate(frames):
            ...
            if self.use_checkpoint and frame_index < len(frames) - 2:
                def fn(frame, *args):
                    ...
                    return (
                        ...
                        #####################################
                        # (25) Adding pred_masks to frame_res
                ...
                ##########################################################
                # () TODO: needs to be manipulated in terms of pred_masks
                frame_res = {
                    'pred_logits': tmp[0],
                    'pred_boxes': tmp[1],
                    'ref_pts': tmp[2],
            ...
            #####################################################################
            # (26) Appending pred_masks to the outputs
...
def build(args):
    ...
    for i in range(num_frames_per_batch):
        weight_dict.update({"frame_{}_loss_ce".format(i): args.cls_loss_coef,
                            ...
                            'frame_{}_loss_giou_bbox'.format(i): args.giou_loss_coef,
                            ####################################################################
                            # (27) Adding masks weight
                            # 'frame_{}_loss_giou_mask'.format(i): args.giou_loss_mask_coef,
                            'frame_{}_loss_mask'.format(i): args.mask_loss_coef,
                            'frame_{}_loss_dice'.format(i): args.dice_loss_coef,
                            # 'frame_{}_combined_loss'.format(i): args.combined_loss_coef,
                            ####################################################################
                            })
        ...
        for i in range(num_frames_per_batch):
            for j in range(args.dec_layers - 1):
                weight_dict.update({"frame_{}_aux{}_loss_ce".format(i, j): args.cls_loss_coef,
                                    ...
                                    'frame_{}_aux{}_loss_giou_bbox'.format(i, j): args.giou_loss_coef,
