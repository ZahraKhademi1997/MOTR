...
class SpatialMLP(nn.Module):
    """Applies an MLP to each spatial position of a 2D image separately."""
    ...
    def forward(self, x):
        ...
        # x = x.view(-1, original_shape[1])  # Flatten spatial dimensions
        x = x.reshape(-1, original_shape[1])

        for i, layer in enumerate(self.layers):
            x = F.relu(layer(x)) if i < self.num_layers - 1 else layer(x)

        # Reshape output to have the same spatial dimensions as input
        x = x.view(original_shape[0], -1, original_shape[2], original_shape[3])
        return x
...
def dice_loss(inputs, targets, size, num_boxes):
    ...
    """
    inputs = inputs.sigmoid() # For segmentation training
    ...
    # output_dir = "/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-AppleMOTS-Axial_Cross_Attention/output/pred_mask/dice_loss"
