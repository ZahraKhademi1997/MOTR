...
#SBATCH --job-name=mot17_MOTR-training
...
# Make sure nproc_per_node matches  gpus-per-node
# PYTHON SCRIPT
#==============
echo "Starting $SLURM_GPUS_PER_TASK process(es) on each node..."
python -m torch.distributed.launch --nproc_per_node=5 \
    ...
    --output_dir /blue/hmedeiros/khademi.zahra/MOTR-train/MOTR-main-MOT17/MOTR-main/output/model_checkpoints/ \
    --batch_size 1 \
    ...
    --extra_track_attn \
    --mot_path /blue/hmedeiros/khademi.zahra/MOTR-train/MOTR-main-MOT17/MOTR-main/data/Dataset/ \
