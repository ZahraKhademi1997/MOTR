...
class DetMOTDetection:
    ...
    def _pre_single_frame(self, idx: int):
        ...
        # print(targets)
        # plot_frame_with_annotations(img, targets, '/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-Query-Selection/output' , 'annotated_image.png')
    ...
    def pre_continuous_frames(self, start, end, interval=1):
        ...
        for i in range(start, end, interval):
            ...
            # print(f"Type of img_i: {type(img_i)}")
    ...
    def __getitem__(self, idx):
        ...
        # print(f"Applying transformation to image list of length: {len(images)}")
        # for image in images:
        ...
        def plot_image_with_bboxes_and_masks(img, bboxes, masks, title):
            ...
            # # Save the figure
            # output_path = os.path.join('/home/zahra/Documents/Projects/prototype/MOTR-codes/DN-DBA-MOTR-Mask/output/plot_joint', file_name)
...
#                     save_failed_image_path = "/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-MOT17-Cross-AA/output/customcropping"  
...
#     save_path = "/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-MOT17-Cross-AA/output/data_aug"
...
# MOTS
def make_transforms_for_mots(image_set, args=None):
    
    ##########################################################################
    # () Defining custom cropping
    class CustomFixedRandomCrop:
        def __init__(self, crop_size, max_attempts=100000000):
            self.crop_size = crop_size
            self.max_attempts = max_attempts
        ...
        def __call__(self, images, all_targets):
            ...
            for img_idx, (image, targets) in enumerate(zip(images, all_targets)):
                ...
                if not valid_crop_found:
                    ...
                    # Save the image that did not contain any valid bbox after all attempts
                    save_failed_image_path = "/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-Query-Selection/output/customcropping"  
                    failed_image_filename = f"failed_image_{img_idx}_{timestamp}.png"  # Include timestamp in the filename
                    failed_image_full_path = os.path.join(save_failed_image_path, failed_image_filename)
    ...
    save_path = "/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-Query-Selection/output/data_aug"
    if not os.path.exists(save_path):
        os.mkdir(save_path)
...
def build_dataset2transform(args, image_set):
    ...
    cococ_train = make_coco_transforms('train', args)
    coco_val = make_coco_transforms('val', args)
    ###############################################################################
    ...
    dataset2transform_train = {'COCO':cococ_train}
    dataset2transform_val = {'COCO':coco_val}
     ###########################################################################################
