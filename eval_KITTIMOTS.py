...
class MOTR(object):
    ...
    def update(self, dt_instances: Instances):
        ...
        for i in range(len(dt_instances)):
            ...
            ret.append(np.concatenate((box_with_score, [id + 1], [label])).reshape(1, -1))
    ...
    if osp.isfile(combined_path):
        ...
        with open(combined_path, 'r') as f:
            for line in f:
                ...
                targets['labels'].append(class_id)  # Assuming single class for simplicity
...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'KITTI_MOTS/train/images', self.seq_num, 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'KITTI_MOTS/train/images', self.seq_num, 'img1', _) for _ in img_list if
                    ('jpg' in _) or ('png' in _)]
        
        # img_list = os.listdir(os.path.join(self.args.mot_path, 'KITTIMOTS/test/images', self.seq_num, 'img1'))
        # img_list = [os.path.join(self.args.mot_path, 'KITTIMOTS/test/images', self.seq_num, 'img1', _) for _ in img_list if
        #             ('jpg' in _) or ('png' in _)]
    ...
    @staticmethod
    def write_results(txt_path, frame_id, bbox_xyxy, identities,labels):
        save_format = '{frame},{id},{label},{x1},{y1},{w},{h},1,-1,-1,-1\n'
        with open(txt_path, 'a') as f:
            for xyxy, track_id,label in zip(bbox_xyxy, identities,labels):
                ...
                x1, y1, x2, y2 = xyxy
                w, h = x2 - x1, y2 - y1
                line = save_format.format(frame=int(frame_id), id=int(track_id),label=label, x1=x1, y1=y1, w=w, h=h)
                f.write(line)

    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'KITTI_MOTS/train/images')
        # data_root = os.path.join(self.args.mot_path, 'KITTIMOTS/test/images')
    ...
    def detect(self, prob_threshold=0.9, area_threshold=300, vis=False):
        total_dts = 0
