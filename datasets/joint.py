...
class DetMOTDetection:
    ...
    def _pre_single_frame(self, idx: int):
        ...
        mask_path = self.mask_files[idx]  # Assuming you have a way to access mask_files similar to img_files and label_files
        ...
        for label in labels:
            obj_id = int(label[1]) + obj_idx_offset if label[1] >= 0 else int(label[1])
            bbox = label[2:6]  # Assuming bbox is already in the correct format
            cropped_mask = mask_np[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]

            # Check if the cropped area contains any part of the mask
            if cropped_mask.any():
                # Create a new mask for the bounding box area
                new_mask = np.zeros(mask_np.shape[:2], dtype=np.uint8)
                new_mask[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])] = (cropped_mask == obj_id).astype(np.uint8)

                targets['boxes'].append(label[2:6].tolist())
                targets['area'].append((bbox[2] - bbox[0]) * (bbox[3] - bbox[1]))
                targets['iscrowd'].append(0)
                targets['labels'].append(0)  # Assuming single class for simplicity
                targets['obj_ids'].append(obj_id)
                targets['masks'].append(torch.tensor(new_mask, dtype=torch.uint8))
        ...
        # print('targets[obj_ids] in joint.py has the shape of:', targets['obj_ids'])
        # print('targets[masks] in joint.py has the shape of:', targets['masks'])  --> Binary 
         ##########################################################################
        # ()
        # Plotting to make sure of getting all masks and bboxes for each image
        # masks = targets['masks'].cpu().numpy()
        # bboxes = targets['boxes'].cpu().numpy()
        # # Plot the image with masks
        # def plot_masks(image, masks, bboxes):
        #     # Plot the image
        #     plt.figure(figsize=(8, 8))
        #     plt.imshow(image)

        #     # Plot masks on top of the image
        #     for mask in masks:
        #         # for msk in mask:
        ...
        # # Call the plot_masks function
        # plot_masks(img, masks, bboxes)
        ##########################################################################
    ...
    def __getitem__(self, idx):
        ...
        def plot_image_with_bboxes_and_masks(img, bboxes, masks, title):
            ...
            # Save the figure
            output_path = os.path.join('/blue/hmedeiros/khademi.zahra/MOTR_graph/model_graph/output/plot_check_joint', file_name)
...
def make_transforms_for_applemots(image_set, args=None):
        ...
        def __call__(self, images, all_targets):
            ...
            for img_idx, (image, targets) in enumerate(zip(images, all_targets)):
                ...
                if not valid_crop_found:
                    ...
                    save_failed_image_path = "/blue/hmedeiros/khademi.zahra/MOTR_graph/model_graph/output/customcropping"  
    ...
    save_path = "/blue/hmedeiros/khademi.zahra/MOTR_graph/model_graph/output/data_aug"
