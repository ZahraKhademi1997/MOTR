...
class DetMOTDetection:
    ...
    def _pre_single_frame(self, idx: int):
        ...
        def plot_frame_with_annotations(img, targets, output_dir, base_filename):
            """
        ...
        elif 'KITTI_MOTS' in img_path:
            targets['dataset'] = 'KITTI_MOTS'
        else:
        ...
        if osp.isfile(label_path):
            # print("File exists, trying to open:", label_path)
            with open(label_path, 'r') as f:
                ...
                # for line in lines:
                    # if line is not None:
                    # if line.strip():  # Ensure line has content
                    #     print('line:', line) 
                    ...
                    if class_id != 10:
                        ...
                        normalized_bbox = list(map(float, parts[3:7])) 
                        rle_str = parts[7]
        ...
        targets['masks'] = [torch.from_numpy(mask) for mask in targets['masks'] if isinstance(mask, np.ndarray) and mask.size > 0]
        if targets['masks']:
            targets['masks'] = torch.stack(targets['masks'])
        else:
            targets['masks'] = torch.zeros(0, int(h), int(w), dtype=torch.uint8)
        # plot_frame_with_annotations(img, targets, '/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_KITTIMOTS_bbox/output/gt' , 'annotated_image.png')
        return img, targets
    ...
    def __getitem__(self, idx):
        ...
        def plot_image_with_bboxes_and_masks(img, bboxes, masks, title):
            ...
            # Save the figure
            output_path = os.path.join('/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_KITTIMOTS_bbox/output/plot_check_joint', file_name)
...
# MOTS
def make_transforms_for_kittimots(image_set, args=None):
    
    ##########################################################################
    # () Defining custom cropping
    class CustomFixedRandomCrop:
        def __init__(self, crop_size, max_attempts=1000000):
        # def __init__(self, crop_size, max_attempts=1):
        ...
        def __call__(self, images, all_targets):
            ...
            for img_idx, (image, targets) in enumerate(zip(images, all_targets)):
                ...
                if not valid_crop_found:
                    ...
                    save_failed_image_path = "/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_KITTIMOTS_bbox/output/customcropping"  
    ...
    save_path = "/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_KITTIMOTS_bbox/output/data_aug"
...
def build_dataset2transform(args, image_set):
    ...
    kittimots_train = make_transforms_for_kittimots('train', args)
    kittimots_val = make_transforms_for_kittimots('val', args)
    ###############################################################################
    ...
    dataset2transform_train = {'MOT17': mot17_train, 'CrowdHuman': crowdhuman_train, 'KITTI_MOTS':kittimots_train}
    dataset2transform_val = {'MOT17': mot17_test, 'CrowdHuman': mot17_test, 'KITTI_MOTS':kittimots_val}
     ###########################################################################################
