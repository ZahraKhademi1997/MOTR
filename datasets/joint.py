...
class ClipMatcher(SetCriterion):
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        if len(active_track_masks_primarly) > 0: 
            ...
            plot_and_save_masks(active_idxes, track_instances.pred_masks, gt_masks, gt_boxes, active_track_boxes, '/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR-train-test/output/criterion')
...
class MOTR(nn.Module):
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord,  outputs_dynamic_params):
        ...
        return [{'pred_logits': a, 'pred_boxes': b, 'pred_mask': c,}
    ...
    def _forward_single_image(self, samples, track_instances: Instances):
        ...
        # print('pred_masks:', pred_masks.shape) # torch.Size([1, 300, 96, 150])
        # print('pred_mask:', pred_masks)
        # outputs_masks = F.interpolate(outputs_mask, size=attn_mask_target_size, mode="bilinear", align_corners=False)
        ...
        #     size=srcs[0].shape[-2:],  # Use the spatial dimensions of srcs[0] --> make sure it is consistence with the embedding from per pixel embedding
        #     mode='nearest'  # Use nearest neighbor interpolation to avoid non-integer results
        # )
        # p3_mask = interpolated_mask_float.to(torch.bool).squeeze(0)  
        # self.p3_mask = p3_mask
        # # print('self.p3_mask:', self.p3_mask.shape) # torch.Size([1, 84, 117])
        # # print('self.p3_mask:', self.p3_mask) # All False
        # num_imgs = outputs_class[-1].size(0)
        # for i in range(num_imgs): 
        #     pos_dynamic_params = outputs_dynamic_params[-1][i]
        #     # print('pos_dynamic_params:', pos_dynamic_params.shape) # torch.Size([1, 300, 256]) 
        #     pos_bbox_preds = outputs_coord[-1][i]
        #     pos_cxcy_coord = pos_bbox_preds[:, :2]
            # print('pos_cxcy_coord:', pos_cxcy_coord.shape) # torch.Size([300, 2])

            # for j in range(pos_dynamic_params.size(0)):
            #     seg_pos_embed = self.mask_positional_encoding(
            #                     self.p3_mask, pos_cxcy_coord[j])
            #     print('seg_pos_embed before:', seg_pos_embed.shape) # torch.Size([1, 8, 116, 182])
                # seg_pos_embed = self.spatial_mlp (seg_pos_embed)
                # print('seg_pos_embed:', seg_pos_embed.shape) # torch.Size([1, 256, 116, 182])
                # seg_pos_embed = seg_pos_embed.flatten(2).transpose(
                #         1, 2)
                # seg_pos_embed = seg_pos_embed.view(seg_pos_embed.shape[1], seg_pos_embed.shape[2])
                
                # embeddings_prob = embeddings_prob.flatten(2).transpose(
                #         1, 2)
                # embeddings_prob = embeddings_prob.view(embeddings_prob.shape[1], embeddings_prob.shape[2])
                
                # cross_attended_output = self.pos_cross_attention(
                #     tgt=pos_dynamic_params,
                #     memory=embeddings_prob,
                #     pos=seg_pos_embed,
                # )
                # print('cross_attended_output:', cross_attended_output.shape) # torch.Size([300, 256])
