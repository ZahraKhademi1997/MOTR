...
class DetMOTDetection:
    ...
    def __getitem__(self, idx):
        ...
        def plot_image_with_bboxes_and_masks(img, bboxes, masks, title):
            ...
            # Save the figure
            output_path = os.path.join('/blue/hmedeiros/khademi.zahra/MOTR-train/MOTR_mask_AppleMOTS_train/MOTR-mask-MOT17-Cross-AA/output/plot_check_joint', file_name)
...
def make_transforms_for_mots(image_set, args=None):
    ...
    if image_set == 'train':
        ...
        for stage, transform in transforms_list:
            # wrapped_transform = lambda image, targets, transform=transform, stage=stage: apply_transform_and_debug(transform, image, targets, stage, save_path)
            wrapped_transform = partial(apply_transform_and_debug, transform=transform, stage=stage, save_path=save_path)
            transforms_with_debug.append(wrapped_transform)
